...
*/
#define PHCRYPTOSYM_SW_AES_VENDOR_ID_C                      43           /**< @brief AUTOSAR vendor ID*/
#define PHCRYPTOSYM_SW_AES_AR_RELEASE_MAJOR_VERSION_C       4            /**< @brief AUTOSAR release major version*/
#define PHCRYPTOSYM_SW_AES_AR_RELEASE_MINOR_VERSION_C       3            /**< @brief AUTOSAR release minor version*/
#define PHCRYPTOSYM_SW_AES_AR_RELEASE_REVISION_VERSION_C    1            /**< @brief AUTOSAR release revision*/
#define PHCRYPTOSYM_SW_AES_SW_MAJOR_VERSION_C               1            /**< @brief The major version of the SW module*/
#define PHCRYPTOSYM_SW_AES_SW_MINOR_VERSION_C               0            /**< @brief The minor version of the SW module*/
#define PHCRYPTOSYM_SW_AES_SW_PATCH_VERSION_C               0            /**< @brief The patch version of the SW module*/

/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/

/* Check if current file and phCryptoSym_Sw_Aes.h header file are of the same vendor */
#if (PHCRYPTOSYM_SW_AES_VENDOR_ID_C != PHCRYPTOSYM_SW_AES_VENDOR_ID)
    #error "PHCRYPTOSYM_SW_AES.c and PHCRYPTOSYM_SW_AES.h have different vendor ids"
#endif
/* Check if current file and phCryptoSym_Sw_Aes.h header file are of the same Autosar version */
#if ((PHCRYPTOSYM_SW_AES_AR_RELEASE_MAJOR_VERSION_C    != PHCRYPTOSYM_SW_AES_AR_RELEASE_MAJOR_VERSION) || \
     (PHCRYPTOSYM_SW_AES_AR_RELEASE_MINOR_VERSION_C    != PHCRYPTOSYM_SW_AES_AR_RELEASE_MINOR_VERSION) || \
     (PHCRYPTOSYM_SW_AES_AR_RELEASE_REVISION_VERSION_C != PHCRYPTOSYM_SW_AES_AR_RELEASE_REVISION_VERSION))
    #error "AutoSar Version Numbers of PHCRYPTOSYM_SW_AES.c and PHCRYPTOSYM_SW_AES.h are different"
#endif
/* Check if current file and phCryptoSym_Sw_Aes.h header file are of the same Software version */
#if ((PHCRYPTOSYM_SW_AES_SW_MAJOR_VERSION_C != PHCRYPTOSYM_SW_AES_SW_MAJOR_VERSION) || \
     (PHCRYPTOSYM_SW_AES_SW_MINOR_VERSION_C != PHCRYPTOSYM_SW_AES_SW_MINOR_VERSION) || \
     (PHCRYPTOSYM_SW_AES_SW_PATCH_VERSION_C != PHCRYPTOSYM_SW_AES_SW_PATCH_VERSION))
    #error "Software Version Numbers of phCryptoSym_Sw_Aes.c and phCryptoSym_Sw_Aes.h are different"
#endif
...
#if ((PHCRYPTOSYM_SW_AES_AR_RELEASE_MAJOR_VERSION_C == 4) && (PHCRYPTOSYM_SW_AES_AR_RELEASE_MINOR_VERSION_C == 0))
 #include <MemMap.h>
#else
...
static  CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_sboxTable[256] = {           \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_invSboxTable[256] = {         \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_times2[256] = {  \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_times3[256] = {  \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_times9[256] = {  \
...
#ifndef PH_CRYPTOSYM_SW_ROM_OPTIMIZATION
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_timesB[256] = { \
...
#endif /* PH_CRYPTOSYM_SW_ROM_OPTIMIZATION */
#ifndef PH_CRYPTOSYM_SW_ROM_OPTIMIZATION
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_timesD[256] = { \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_timesE[256] = { \
...
#else
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_timesC[256] = { \
...
static CONST(uint8, ANFCRL_CONST) PH_CRYPTOSYM_SW_CONST_ROM phCryptoSym_Sw_Aes_Rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
...
FUNC(phStatus_t, ANFCRL_CODE) phCryptoSym_Sw_Aes_KeyExpansion(
    ...
    VAR(uint8, ANFCRL_VAR) bNkCurrent,
    VAR(uint8, ANFCRL_VAR) bNkMax
    )
{
#ifndef PH_CRYPTOSYM_SW_ONLINE_KEYSCHEDULING
    VAR(uint8, ANFCRL_VAR) i;
    VAR(uint8, ANFCRL_VAR) rconId = 0;
    P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_MEMLOC_BUF pKeyPtrNmNk = pDataParams->pKey;
    P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_MEMLOC_BUF pKeyPtrNm1 = &pDataParams->pKey[((bNkCurrent) << 2U) - 4];
    P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_MEMLOC_BUF pCurKeyPtr = &pDataParams->pKey[(bNkCurrent) << 2U];
    ...
    */
    ...
    (void)memcpy(pDataParams->pKey, pKey, (size_t)(((uint32)bNkCurrent) << 2U));
...
FUNC(phStatus_t, ANFCRL_CODE) phCryptoSym_Sw_Aes_EncryptBlock(
    ...
    VAR(uint8, ANFCRL_VAR) bNumRounds
    )
{
...
#else
    VAR(uint8, ANFCRL_VAR) i;
    VAR(uint8, ANFCRL_VAR) bNk;
    VAR(uint8, ANFCRL_VAR) bCurrentNk = 0;
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM bTmpKey[PH_CRYPTOSYM_AES256_KEY_SIZE];
    ...
    /* Re-copy the key */
    (void)memcpy(bTmpKey, pDataParams->pKey, (size_t)(((uint32)bNk) << 2U));
    ...
    )
{
    ...
    VAR(uint8, ANFCRL_VAR) i;
    VAR(uint8, ANFCRL_VAR) bNk;
    VAR(uint8, ANFCRL_VAR) bCurrentNk;
    VAR(uint8, ANFCRL_VAR) bTmp;
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM bTmpKey[PH_CRYPTOSYM_AES256_KEY_SIZE];
...
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_SubBytesShiftRows(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState)
{
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM tmp;
...
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_InvSubBytesShiftRows(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState)
{
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM tmp;
...
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_MixColumns(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState)
{
...
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_InvMixColumns(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState)
{
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM tmp[4];
    VAR(uint8, ANFCRL_VAR) i;
...
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_AddRoundKey(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState, P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pKey, P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) pNkCurrent, VAR(uint8, ANFCRL_VAR) bNk, VAR(uint8, ANFCRL_VAR) bMode)
{
    VAR(uint8, ANFCRL_VAR) i = 0u;
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM bOffset;
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM bCurKeyIdx;
    VAR(uint8, ANFCRL_VAR) PH_CRYTOSYM_SW_FAST_RAM bModulus;
    ...
    for (i = 0; i < 4U; i++)
    {
        ...
        /* Generally the bCurKeyIdx can be calculated directly out of the bModulus */
        bCurKeyIdx = bModulus << 2U;

        /* In case of decryption, xoring has to be done upfront. */
        if (bMode == PH_CRYPTOSYM_KEYSCHEDULE_DECRYPTION)
        {
            pState[((3U-i)*4U)] ^= (pKey[bCurKeyIdx + 0u]);
            pState[((3U-i)*4U)+1U] ^= (pKey[bCurKeyIdx + 1u]);
            pState[((3U-i)*4U)+2U] ^= (pKey[bCurKeyIdx + 2u]);
            pState[((3U-i)*4U)+3U] ^= (pKey[bCurKeyIdx + 3u]);
        }

        if ((*pNkCurrent) < bNk)
        {
            /* w[i] = word(key[4U*i], key[4U*i+1U], key[4U*i+2U], key[4U*i+3U]) */
            /* Just increment the current key index, as the key is already stored internally...*/
            bCurKeyIdx += 4u;
        }else
        {
            if ( bModulus == 0U)
            {
                ...
                bOffset = (uint8)(((uint8)((uint8)bNk-1U)<<2U) + 1U);
                /* now calculate the following calculation: */
                /* w[i] = w[i-Nk] xor SubWord(RotWord(w[i-1])) xor Rcon[i/Nk] */
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx + bOffset]] ^ phCryptoSym_Sw_Aes_Rcon[((*pNkCurrent)/bNk) - 1U];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx + bOffset]];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx + bOffset]];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx + bOffset - 4U]];
                bCurKeyIdx++;
            }else if ((bNk == 8U) && (((*pNkCurrent) % bNk) == 4U))
            {
                /* w[i] = w[i-Nk] xor SubWord(w[i-1]) */
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx - 4u]];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx - 4u]];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= phCryptoSym_Sw_Aes_sboxTable[pKey[bCurKeyIdx - 4u]];
            ...
            }else
            {
                ...
                pKey[bCurKeyIdx] ^= pKey[bCurKeyIdx - 4u];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= pKey[bCurKeyIdx - 4u];
                bCurKeyIdx++;
                pKey[bCurKeyIdx] ^= pKey[bCurKeyIdx - 4u];
        ...
        {
            pState[(i*4U)] ^= pKey[bCurKeyIdx - 4u];
            pState[(i*4U)+1U] ^= pKey[bCurKeyIdx - 3u];
            pState[(i*4U)+2U] ^= pKey[bCurKeyIdx - 2u];
            pState[(i*4U)+3U] ^= pKey[bCurKeyIdx - 1u];
        }
    }
}
#endif /* PH_CRYPTOSYM_SW_ONLINE_KEYSCHEDULING */
...
#ifndef PH_CRYPTOSYM_SW_ONLINE_KEYSCHEDULING
FUNC(void, ANFCRL_CODE) phCryptoSym_Sw_Aes_AddRoundKey(P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_CRYTOSYM_SW_FAST_RAM pState, P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_MEMLOC_BUF pKey, VAR(uint8, ANFCRL_VAR) bCnt)
{
    ...
    P2VAR(uint8, AUTOMATIC, ANFCRL_APPL_DATA) PH_MEMLOC_BUF pRoundKey = &pKey[bCnt << 4U];
