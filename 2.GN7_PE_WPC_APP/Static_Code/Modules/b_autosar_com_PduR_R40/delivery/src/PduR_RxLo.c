...
#endif
{
  ...
  { 
    ...
    firstQueue = &PduR_GaaTxFirstQueue[LpGWIfPdu->usFirstQueueIndex];
	lastQueue = &PduR_GaaTxLastQueue[LpGWIfPdu->usFirstQueueIndex];
  }
  #else  
  {
  ...
  if (((uint8)PDUR_D_FIFO == LucMask)||((uint8)PDUR_TT_FIFO == LucMask))
  {
	...
	{ 
		...
		if ((LpTxSharedBuffer->uCurrDepth) >= (LpTxSharedBuffer->uBufferDepth))
		{
	      ...
	      while(ptrQueue->Next != NULL_PTR)
	      {
			nextQueue = ptrQueue->Next;
			
			ptrQueue->Next = NULL_PTR;
			ptrQueue->FreeBuffer = (boolean)PDUR_FALSE;
			ptrQueue->TargetPduId = 0;
		...
		#if(PDUR_CANIF_SUPPORT == STD_ON)
		if(((uint8)PDUR_D_FIFO == LucMask) && (NULL_PTR != firstQueue->Next))
		{
		  ...
		  if(LucCanIfPduStatus == CANIF_GW_NOREQUEST)
		  {
			...
			LpBufPduInfoPtr = &(ptrQueue->PduInfoPtr);
		  ...
		  CANIF_GW_ERRVOL))
		  {
			...
			/* Free all queue about same targetPduId */ 
		...
		indexDepth = (uint16)PDUR_ZERO;
  	    do
		{
		  ptrQueue = &PduR_GaaTxQueue[LpGWIfPdu->usTxBufferIndex + indexDepth];
		  indexDepth++;
		}
		while ((indexDepth < LpTxSharedBuffer->uBufferDepth) &&
		  ((boolean)PDUR_TRUE == ptrQueue->FreeBuffer)); 
  ...
  if ((LucMask == (uint8)PDUR_D_FIFO) || (LucMask == (uint8)PDUR_TT_FIFO))
  {
    ...
    #endif
    {
	  ...
	  if (PDUR_INVALID != LpGWIfPdu->ucTxSharedBufferIndex)
	  {	
        if(NULL_PTR == firstQueue->Next) firstQueue->Next = ptrQueue;
		
		if(NULL_PTR == lastQueue->Next) lastQueue->Next = ptrQueue;
		else 
		{
		  nextQueue = lastQueue->Next;
		  
		  nextQueue->Next = ptrQueue;
		  lastQueue->Next = ptrQueue;		  
		}
		
	    /* Update TargetPduId for later use (in TxConfirmation context) */
	    ptrQueue->TargetPduId = LpGWIfPdu->ddTargetPduId;
	    /* set flag FreeBuffer */
	    ptrQueue->FreeBuffer = (boolean)PDUR_TRUE;
	    ptrQueue->Next = NULL_PTR;
  ...
  if (LddPduRSrcPduId < PDUR_LO_RXINDICATION_MAXPDUID)
  {
    ...
    while ((uint8)PDUR_ZERO != LucNoOfRoutePdu)
    {
      ...
      if ((uint8)PDUR_D_FIFO == LucMask)
      {
        ...
        LucConfiguredFifoDepth = LpTxSharedBuffer->uBufferDepth;
      
        /* Get current TxBuffer */
        LucCurrentFifoDepth = LpTxSharedBuffer->uCurrDepth;
        #else
        LpTxBuffer = &PduR_GaaTxBuffer[LpGWIfPdu->usTxBufferIndex];
