    ...
    PDUR_APPL_CONST) ConfigPtr)
{
    ...
    LusLoopIndex++)
  {
    for(LusLoop = PDUR_ZERO; LusLoop < ((uint16)PduR_GaaTxSharedBuffer[LusLoopIndex].uBufferDepth);
      LusLoop++)
    {
	  PduR_GaaTxQueue[LusIndex].PduInfoPtr.SduDataPtr =
			  &(PduR_GaaTxSharedBuffer[0].SduDataPtr)[LusSum];
      PduR_GaaTxQueue[LusIndex].PduInfoPtr.SduLength =
	  	      PduR_GaaTxSharedBuffer[LusLoopIndex].SduLength;
      LusIndex++;
  ...
  LusLoopIndex++)
  {
    PduR_GaaTxQueue[LusLoopIndex].FreeBuffer = (boolean)PDUR_FALSE;
    PduR_GaaTxQueue[LusLoopIndex].Next =   NULL_PTR;
	PduR_GaaTxQueue[LusLoopIndex].TargetPduId = 0;
  }
  
  for(LusLoopIndex = PDUR_ZERO; LusLoopIndex < ((uint16)PDUR_TX_FIRSTQUEUE_INDEX_COUNT);
  LusLoopIndex++)
  {
    PduR_GaaTxFirstQueue[LusLoopIndex].FreeBuffer = (boolean)PDUR_FALSE;
	PduR_GaaTxFirstQueue[LusLoopIndex].Next = NULL_PTR;

	PduR_GaaTxLastQueue[LusLoopIndex].FreeBuffer = (boolean)PDUR_FALSE;
	PduR_GaaTxLastQueue[LusLoopIndex].Next = NULL_PTR;
  }  
  #endif
