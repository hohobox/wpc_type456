...
FUNC(void, DEM_CODE) Dem_EvMem_ResetEventStatusNvRamBlock(void)
{
  ...
  */
  ...
  for(nvDataIndex = 0u; nvDataIndex < DEM_NUMBER_OF_RATIO_INFO; nvDataIndex++)
  {
  ...
  {
    Dem_EventStatusNvRamData.IumprUpdateFlagPerGroup[LucGroupRatioCount] = DEM_FALSE;
    Dem_EventStatusNvRamData.IumprMinimumRatioIDPerGroup[LucGroupRatioCount] =  (uint16)DEM_ZERO;
  }

  #endif /* DEM_NUMBER_OF_RATIO_INFO > 0U */

  Dem_EventStatusNvRamData.DataOfPID21 = (uint16)DEM_ZERO;
  Dem_EventStatusNvRamData.DataOfPID31 = (uint16)DEM_ZERO;
  Dem_EventStatusNvRamData.DataOfPID4D = (uint16)DEM_ZERO;
  Dem_EventStatusNvRamData.DataOfPID4E = (uint16)DEM_ZERO;
  Dem_EventStatusNvRamData.MilonFlag = DEM_ZERO;
  Dem_EventStatusNvRamData.PreMilonFlag = DEM_ZERO;
...
*
*****************************************************************************/
/* polyspace+3 MISRA-C3:8.13 [Justified:Low] "entry is depending on If-condition and it evaluates to true or false according to configuration"*/ 
FUNC(boolean, DEM_CODE)
  ...
  boolean EvStatusIsDirty)
{
  ...
  #endif
  {
    ...
    if ( memLoc < Dem_SizeOfEventMemory[DTCOrigin])
    {
      ...
      /* polyspace +3 MISRA-C3:2.1 [Not a Defect:Low] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
	  /* polyspace +2 MISRA-C3:14.3 [Not a Defect:Low] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
      /* polyspace +1 DEFECT:DEAD_CODE [Not a defect:Justified] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
      if (DEM_TRUE == storageTestFailed)
      {
        mask = DEM_UDS_STATUS_MASK_EVMEM | ((Dem_UdsStatusByteType)DEM_UDS_STATUS_TF);
      }
	  /* polyspace +1 MISRA-C3:2.1 [Not a Defect:Low] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
      else /* FALSE */
      {
    ...
    {    
	  ...
	  /* polyspace +2 MISRA-C3:14.3 MISRA-C3:2.1 [Not a Defect:Low] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
      /* polyspace +1 DEFECT:DEAD_CODE [Not a defect:Justified] "It is depend on configuration Dem Event. This implementation depend on the configuration DemEventStatusBitStorageTestFailed. User can configure this parameter true or false. No Impact." */
      ...
      if (DEM_DTC_ORIGIN_PRIMARY_MEMORY == DTCOrigin)
      {
        for (memLoc = 0U; memLoc < numOfMemEntries; memLoc++)    
        {
          ...
          if(DEM_NO_EVENT == Dem_PrimaryEventMemory[memLoc].EventIdRef)
          {
            ...
            if (cbDTC != NULL_PTR)
            {
              if ((DEM_TRUE == Dem_EvMem_DTCFormatIsValid(DEM_DTC_FORMAT_OBD, cbDTC)) && 
                  (DEM_NO_EVMEM_ENTRY == Dem_ObdMemoryId))
              {
                 SchM_Enter_Dem_REQUEST_STATUS_PROTECTION();
                 Dem_ObdMemoryId = memLoc; //store the index of Dem_PrimaryEventMemory
                 Dem_EventStatusNvRamData.ObdMemoryId = Dem_ObdMemoryId;
	
				*StatusDirty = DEM_TRUE;
				 SchM_Exit_Dem_REQUEST_STATUS_PROTECTION();
              }
            }
            #endif /*DEM_OBD_SUPPORT == STD_ON */
 ...
 ** Remarks             : 
 *******************************************************************************/
/* polyspace+5 MISRA-C3:8.13 [Justified:Low] "EvMemEt is needed for comparision in IF condition. No impact" */
