...
typedef struct STag_Dem_OBDClassType
{
  ...
  /* An array of the Data Element */
  P2CONST(Dem_PidRecordType, AUTOMATIC, DEM_CONST) PidClass;
}Dem_OBDClassType;
#endif
...
typedef struct STag_Dem_EventParameterType 
{
  ...
  */
  ...
  /*uint16 usObdDtc;*/ /*not use and not generated*/
...
{ 
  ...
  uint16 IumprMinimumRatioIDPerGroup[DEM_SIXTEEN];
  ...
  boolean MonitorActivityDenominatorNeedCountedCycleEnd[DEM_NUMBER_OF_RATIO_INFO];
  /* Store the indicator of Monitor Activity Numerator whether it is updated in one Operation Cycle*/
  boolean MonitorActivityNumeratorNeedCountedCycleEnd[DEM_NUMBER_OF_RATIO_INFO];
  
  boolean IumprUpdateFlagPerGroup[DEM_SIXTEEN];
 ...
 ** Array of ratio Id Info                                                    **
 *******************************************************************************/
#if((DEM_NUMBER_OF_DTRS > DEM_ZERO) && (DEM_OBD_SUPPORT == STD_ON))
