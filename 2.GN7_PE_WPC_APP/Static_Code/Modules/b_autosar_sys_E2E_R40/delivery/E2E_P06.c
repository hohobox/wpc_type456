...
#define E2E_P06_C_AR_RELEASE_MAJOR_VERSION            (4)
#define E2E_P06_C_AR_RELEASE_MINOR_VERSION            (2)
#define E2E_P06_C_AR_RELEASE_REVISION_VERSION         (2)

/* FILE VERSION INFORMATION */
#define E2E_P06_C_SW_MAJOR_VERSION                    (1)
#define E2E_P06_C_SW_MINOR_VERSION                    (2)
#define E2E_P06_C_SW_PATCH_VERSION                    (5)

/*******************************************************************************
**                      Version Check                                         **
*******************************************************************************/
#if (E2E_AR_RELEASE_MAJOR_VERSION != E2E_P06_C_AR_RELEASE_MAJOR_VERSION)
#error "E2E_P06.c : Mismatch in Specification Major Version"
#endif

#if (E2E_AR_RELEASE_MINOR_VERSION != E2E_P06_C_AR_RELEASE_MINOR_VERSION)
#error "E2E_P06.c : Mismatch in Specification Minor Version"
#endif

#if (E2E_AR_RELEASE_REVISION_VERSION != E2E_P06_C_AR_RELEASE_REVISION_VERSION)
#error "E2E_P06.c : Mismatch in Specification Revision Version"
#endif

#if (E2E_SW_MAJOR_VERSION != E2E_P06_C_SW_MAJOR_VERSION)
#error "E2E_P06.c : Mismatch in Major Version"
#endif

#if (E2E_SW_MINOR_VERSION != E2E_P06_C_SW_MINOR_VERSION)
#error "E2E_P06.c : Mismatch in Minor Version"
#endif

#if (E2E_SW_PATCH_VERSION != E2E_P06_C_SW_PATCH_VERSION)
#error "E2E_P06.c : Mismatch in Patch Version"
#endif
/*******************************************************************************
**                      Global Data Types                                     **
*******************************************************************************/
  ...
  VAR(uint16, AUTOMATIC) Length)
{
  ...
  if (Length >= (LusOffset + E2E_P06_LEN_OFS_HIGH_BYTE))
  {
	 
    /* Data length after the position where CRC value exist */
    LusLengthAftCrc = (Length - LusOffset) - E2E_P06_LEN_OFS_HIGH_BYTE;
    ...
    if (E2E_ZERO < LusOffset)
    {
      ...
      LusCRCValue = Crc_CalculateCRC16 (
        &DataPtr[0], (uint32)LusOffset, E2E_P06_CRC_START_VAL, 
        E2E_TRUE);
    
      /* calculate CRC after the CRC position */
      LusCRCValue = Crc_CalculateCRC16 (
        &DataPtr[LusOffset + E2E_P06_LEN_OFS_HIGH_BYTE], (uint32)LusLengthAftCrc,
    ...
    else
    {
      ...
      LusCRCValue = Crc_CalculateCRC16 (
        &DataPtr[LusOffset + E2E_P06_LEN_OFS_HIGH_BYTE], (uint32)LusLengthAftCrc,
        E2E_P06_CRC_START_VAL, E2E_TRUE);
    } /* End of else */  
  }
  else
  {
...
** Remarks              : Global Variable(s)     : None                       **
**                                                                            **
**                        Function(s) invoked    : E2E_P06CalculateCRC        **
*******************************************************************************/
  ...
  VAR(uint16, AUTOMATIC) Length)
{
  ...
  else if ((((ConfigPtr->MinDataLength) / E2E_DIVIDE_BY_EIGHT) > Length) ||
           (((ConfigPtr->MaxDataLength) / E2E_DIVIDE_BY_EIGHT) < Length))
  {
    LddRetVal = E2E_E_INPUTERR_WRONG;
  }
  else if ((NULL_PTR != DataPtr) && (Length < ((ConfigPtr->Offset / E2E_DIVIDE_BY_EIGHT) + E2E_P06_LEN_OFS_HIGH_BYTE)))
  {
	LddRetVal = E2E_E_INPUTERR_WRONG;
  }
  else
  {
    ...
    DataPtr[LusOffset + E2E_P06_LEN_OFS_HIGH_BYTE] =
    (uint8)((Length >> E2E_SHIFT_BY_EIGHT) & E2E_BYTE_MASK);
    
    DataPtr[LusOffset + E2E_P06_LEN_OFS_LOW_BYTE] =
    (uint8)(Length & E2E_BYTE_MASK);
  
    /* Write Counter */
    DataPtr[LusOffset + E2E_P06_CNT_OFS] = StatePtr->Counter;
    
    /* Compute CRC */
    LusCrcVal = E2E_P06CalculateCRC (
    ...
    DataPtr[LusOffset + E2E_P06_CRC_OFS_LOW_BYTE] = 
      (uint8)(LusCrcVal & E2E_BYTE_MASK);
    
    /* Increment Counter */
    StatePtr->Counter++;
  } /* End of if-elseif-else */
  return LddRetVal;
} /* End of E2E_P06Protect (
   * P2CONST(E2E_P06ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
   * P2VAR(E2E_P06ProtectStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,
   * P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr,
   * VAR(uint16, AUTOMATIC) Length)
  ...
  VAR(uint16, AUTOMATIC) Length)
{
  ...
  VAR(uint16, AUTOMATIC) LusRcvdLenHighByteVal;
  VAR(uint16, AUTOMATIC) LusRcvdLenLowByteVal;
  VAR(uint16, AUTOMATIC) LusRcvdLenVal;
  ...
  /* polyspace<MISRA2012:D4.14:Not a defect:Justify with annotations> Validity is checked from AUTOSAR SPEC  */
      ((((ConfigPtr->MinDataLength) / E2E_DIVIDE_BY_EIGHT) > Length) ||
      (((ConfigPtr->MaxDataLength / E2E_DIVIDE_BY_EIGHT) < Length))))
  {
    LddRetVal = E2E_E_INPUTERR_WRONG;
  } /*
  ...
  else
  {
    ...
    else
    {
      ...
      /* Get the high byte of Len value on the received data */
      LusRcvdLenHighByteVal = 
        ((uint16)(DataPtr[LusOffset + E2E_P06_LEN_OFS_HIGH_BYTE])) << 
        E2E_SHIFT_BY_EIGHT;
      
      /* Get the low byte of Len value on the received data */
      LusRcvdLenLowByteVal = 
        (uint16)(DataPtr[LusOffset + E2E_P06_LEN_OFS_LOW_BYTE]);

      /* 
       * Get the Len value from high and low byte of Len position on received 
       * data 
       */
      LusRcvdLenVal = 
        (LusRcvdLenHighByteVal & E2E_HIGH_BYTE_MASK) |
        (LusRcvdLenLowByteVal & E2E_BYTE_MASK);
    
      
      /* Read Counter */
      LucRcvdCntrVal = DataPtr[LusOffset + E2E_P06_CNT_OFS];

      /* Read CRC */

      /* Get the high byte of CRC value on the received data */
      LusRcvdCrcHighByteVal = 
        ((uint16)(DataPtr[LusOffset])) << E2E_SHIFT_BY_EIGHT;
      
      /* Get the low byte of CRC value on the received data */
      LusRcvdCrcLowByteVal = 
        (uint16)(DataPtr[LusOffset + E2E_P06_CRC_OFS_LOW_BYTE]);
      ...
      /* Compute CRC */
      LusCptdCrcVal = E2E_P06CalculateCRC (ConfigPtr, DataPtr, Length);

      /* Do checks */
      
      /* when the received crc is not equal to the computated crc */ 
      ...
      /* when the received length is not equal to transmitted length */
      else if (LusRcvdLenVal != Length)
      {
		/* polyspace<MISRA2012:D4.14:Not a defect:Justify with annotations> Validity is checked from AUTOSAR SPEC  */
        StatePtr->Status = E2E_P06STATUS_ERROR;
      } /* End of else if (LusRcvdLenVal != Length) */
      else
      {
        ...
        {
          LucDeltaCnt = 
            ((E2E_P06_CNT_MAX - StatePtr->Counter) + LucRcvdCntrVal) + E2E_ONE;
        }
   ...
   * End of E2E_P06Check (
   * P2VAR(E2E_P06ConfigType, AUTOMATIC, E2E_APPL_DATA) Config,
   * P2VAR(E2E_P06ReceiverStateType, AUTOMATIC, E2E_APPL_DATA) State,
  ...
  P2VAR(E2E_P06CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr)
{
  ...
  else
  {
    ...
    /* Set Status to E2E_P06STATUS_ERROR */
    StatePtr->Status = E2E_P06STATUS_ERROR;
  } /* End of if-else */
  
  return LddRetVal;
} /* End of E2E_P06CheckInit (
   * P2VAR(E2E_P06CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr)
   */
