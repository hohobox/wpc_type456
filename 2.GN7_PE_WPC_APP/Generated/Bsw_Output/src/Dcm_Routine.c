...
P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_APPL_DATA) pMsgContext)
{
  ...
  if((LddReturnValue == E_OK) && (NULL_PTR != LpRoutineSignalInfo))
  {
    ...
    else if(LucSubFunction == DCM_ROUTINE_CTRL_REQUEST_RESULT)
    {
      ...
      {
        (void)Dcm_RequestResultRoutinePackSignal(LusRIDIndex, pMsgContext);
      }
    }
    #endif
    else
        ...
        else
        {
          ...
          if(LblSearchResult == DCM_TRUE)
            {  
              ...
              /* Check the current authentication Role supported by the service Id */
              uint8 LucCtrlType = pMsgContext->reqData[DCM_ZERO] & DCM_SUBFUNCTION_MASK;
              Dcm_RoutineConfigType* LpRoutineCfg = &Dcm_GaaRoutineControlConfig[LusRIDIndex];
              LblSearchResult = Dcm_RidCtrl_AuthValidate(LucCtrlType, LusRoutineIdentifier, LpRoutineCfg);
              #endif

            /* polyspace-begin DEFECT:DEAD_CODE MISRA-C3:2.1 MISRA-C3:14.3 [Justified:Low] "LblSearchResult can be changed when Authentication Support." */
              ...
              if(LblSearchResult == DCM_TRUE)
              {
                ...
                #endif 
                {
                  ...
                  else
                  {
                    ...
                    {
                      LddReturnValue = Dcm_StopRoutineUnPackSignal(LusRIDIndex, pMsgContext);
                    }
            ...
            {
              /* Report the Authentication Required NRC */
              Dcm_GddNegRespError = DCM_E_AUTHENTICATIONREQUIRED;
            } 
            /* polyspace-end DEFECT:DEAD_CODE MISRA-C3:2.1 MISRA-C3:14.3 [Justified:Low] "LblSearchResult can be changed when Authentication Support." */ 
          }
          else
          {
            /* Report the Request out of range NRC */
...
**                                                                            **
*******************************************************************************/
/* polyspace+4 MISRA-C3:8.13 [Justified:Low] "routineCfg depends on user configuration." */
  ...
  Dcm_RoutineConfigType* routineCfg)
{
  ...
  boolean LddRetVal = DCM_FALSE;
  uint8 idx;

  /*To copy RID Role*/
  uint8 LddSize = DCM_AUTHENTICATION_ROLE_SIZE;
  switch (ctrlType)
  {
    case DCM_ROUTINE_CTRL_START:
      for (idx = 0U; idx < LddSize; idx++)
      {
        LddRole[idx] = routineCfg->StartRoutineRole[idx];
      }
      break;
    case DCM_ROUTINE_CTRL_STOP:
      for (idx = 0U; idx < LddSize; idx++)
      {
        LddRole[idx] = routineCfg->StopRoutineRole[idx];
      }
      break;
    case DCM_ROUTINE_CTRL_REQUEST_RESULT:
      for (idx = 0U; idx < LddSize; idx++)
      {
        LddRole[idx] = routineCfg->RoutineResultsRole[idx];
      }
      break;
    default:
      for (idx = 0U; idx < LddSize; idx++)
  ...
  /* Check RID AuthenticationRole */
  if (DCM_FALSE == Dcm_ValidateAccessRights(LddRole))
  {
    /* Check RID AuthenticationRole with WhiteList */
    ...
    if(NULL_PTR != LpRoutineSignalInfo)
    {
         ...
         (NULL_PTR != LpRoutineSignalInfo->pRequestResOut))
      {
        ...
        {    
          ...
          (LpRoutineOutSignalInfo[LusSignalCount].usSignalStartByte)];
...
P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_APPL_DATA) pMsgContext)
{
  ...
  LpRIDTAB = &Dcm_GaaRoutineControlConfig[LusRIDIndex];
  /*before enter the Dcm_StartRoutinePackSignal function, it alreay check the LpRIDTAB->pRoutineSignalInfo is null or not  */
  LpRoutineSignalInfo = LpRIDTAB->pRoutineSignalInfo;
 ...
 /*before enter the Dcm_StartRoutinePackSignal function,LpRoutineSignalInfo->ucNumOfStartRtnOutSignal and  LpRoutineSignalInfo->pStartRoutineOut value is already checked. */
  ...
  {      
    ...
    LpResponseData = 
      &pMsgContext->resData[DCM_THREE + (LpRoutineOutSignalInfo[LusSignalCount].usSignalStartByte)];
...
P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_APPL_DATA) pMsgContext)
{
  ...
  /*before enter the Dcm_StartRoutinePackSignal function,LpRoutineSignalInfo->ucNumOfStopRtnOutSignal and  LpRoutineSignalInfo->pStopRoutineOut value is already checked. */
  /* Initialize pointer to the stop Routine In Signal */
  ...
  if(LucSubFunction == DCM_ROUTINE_CTRL_START)
  {
    switch(LusRoutineIdentifier)
    {
      ...
      case 512 :
				...
				LddReturnValue = Fota_Start_CheckMemory(LddOpStatus,
				&LddNegativeErrorCode);
				break;

      case 528 :

				LddReturnValue = Fota_Start_ReadActiveArea(
      ...
      case 529 :
				...
				LddReturnValue = Fota_Start_FinishUpdate(LddOpStatus,
				&LddNegativeErrorCode);
				break;

      case 530 :

				LddReturnValue = Fota_Start_EraseTargetArea(
				(P2VAR(uint8, AUTOMATIC, DCM_PRIVATE_DATA)) &Dcm_GaaRoutineSignalInData[0],
				LddOpStatus,
				 &Dcm_GaaRoutineSignalOutData[0],
				&Dcm_GusCurrentDataLength,&LddNegativeErrorCode);
      ...
      case 531 :
				...
				LddReturnValue = Fota_Start_SwapActiveArea(
				(P2VAR(uint8, AUTOMATIC, DCM_PRIVATE_DATA)) &Dcm_GaaRoutineSignalInData[0],
				LddOpStatus,
				 &Dcm_GaaRoutineSignalOutData[0],
				&Dcm_GusCurrentDataLength,&LddNegativeErrorCode);
      ...
      case 65280 :
				...
				LddReturnValue = Fota_Start_EraseMemory(
				(P2VAR(uint8, AUTOMATIC, DCM_PRIVATE_DATA)) &Dcm_GaaRoutineSignalInData[0],
				LddOpStatus,
				 &Dcm_GaaRoutineSignalOutData[0],
				&Dcm_GusCurrentDataLength,&LddNegativeErrorCode);
      ...
      case 65281 :
				...
				LddReturnValue = Fota_Start_CheckProgrammingDependency(
				(P2VAR(uint8, AUTOMATIC, DCM_PRIVATE_DATA)) &Dcm_GaaRoutineSignalInData[0],
				LddOpStatus,
				 &Dcm_GaaRoutineSignalOutData[0],
				&Dcm_GusCurrentDataLength,&LddNegativeErrorCode);
