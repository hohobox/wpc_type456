...
/* Generate Seed from random generator */
   ...
   defined(Rte_Call_CsmRandomGenerate_HaeHsmPseudoRandomGenerate_L9_RandomGenerate))
...
/* 010A_07 */
// /* WPC_1914_01 Start */
// #if !defined(DEBUG_OEUK_DEV_KEY_USE) 	/* SHLee : PubKey를 App Flash에서 가져다 씀 */
// typedef uint8 PubKey_t[260];
// volatile const PubKey_t* mb_PublicKeyForOEUK = (PubKey_t*)0x10059000U;
// #else 	/* SHLee : 테스트를 위해 PubKey를 상수로 사용 */
// typedef uint8 PubKey_t[260];
// volatile const uint8 raw_PublicKeyForOEUK[260] = {
...
// 	0x01, 0x00, 0x01, 0x00
// };
// volatile const PubKey_t* mb_PublicKeyForOEUK = (PubKey_t*)&raw_PublicKeyForOEUK;
// #endif
// /* WPC_1914_01 Stop */
/* 010A_07 */

/* CYTXXX_Lite.ld 파일에 Section 추가해야함 */
/* code 영역 시작하기 전에 section 추가함. */
/* EcuInfo.h에 위치 시키면 컴파일 에러 발행해서 .C로 이동시킴. */
#pragma ghs section rodata="FBL_VERSION_INFO"	/* flash 저장 위치 고정 */

const uint8 Fbl_Version_Info[16] = {cFblSoftVerYear1, cFblSoftVerYear2, cFblSoftVerMonth, cFblSoftVerOrder, cFblSoftVerUsage, \
                                    cAppSoftVerCAR1, cAppSoftVerCAR2, cAppSoftVerIndex1, cAppSoftVerIndex2, \
                                    cSWP_FBL_VER1,cSWP_FBL_VER2,cSWP_FBL_VER3, \
                                    0x00, 0x00, 0x00, 0x00};

#pragma ghs section				/* 위치 지정 해제 */
  ...
  AppDcm_DiagnosticSessionControlNotification(void)
{
  ...
  if((LddPrevMode != DCM_DEFAULT_SESSION) && (LddNextMode == DCM_DEFAULT_SESSION))
  {
    Mcu_PerformReset();
  }
}
  ...
  IN Dcm_OpStatusType OpStatus)
{
  ...
  result = ASK_KeyGenerate(&AppDcm_GaaRandomNum[0], &LaaKey[0]);
  if (SEEDKEY_SUCCESS == result)
  {
    uint8 LucIndex;
  ...
  Dcm_OpStatusType OpStatus)
{
  ...
  switch(AppDcm_SignatureVerifySequence)
  {
    case SIGNATURE_VERIFY_START:
      ...
      {
        Dcm_GaaPublicKey.data[LucIndex] = ((uint8*)Dcm_GaaPublicKeyData)[LucIndex]; /* 010A_07 */
      }
