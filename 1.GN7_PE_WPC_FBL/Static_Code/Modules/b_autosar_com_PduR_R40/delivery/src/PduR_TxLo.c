...
** 2.9.7     12-Aug-2022   HM Shin     #36766                                 **
...
P2CONST(PduR_LoTxConf, AUTOMATIC, PDUR_CONST) LpLoTxConfirmation)
{
  ...
  P2VAR(PduR_TxNodeType, AUTOMATIC, PDUR_VAR) ptrNode;
  P2VAR(PduR_TxNodeType, AUTOMATIC, PDUR_VAR) firstNode;
  P2VAR(PduR_TxNodeType, AUTOMATIC, PDUR_VAR) lastNode;
  #else
  ...
  if (PDUR_INVALID != LpLoTxConfirmation->ucTxSharedBufferIndex)
  {
    ...
    firstNode = &PduR_GaaTxFirstNode[LpLoTxConfirmation->usFirstNodeIndex];
    lastNode = &PduR_GaaTxLastNode[LpLoTxConfirmation->usFirstNodeIndex];
    ...
    if (LpLoTxConfirmation->ucBufferType == (uint8)PDUR_D_FIFO)
    {
      ...
      {        
        ...
        firstNode->Next = ptrNode->Next; /* polyspace MISRA-C:2012 D4.1 [Not a defect:Low] "initialized pointer due to data flow" */ /* polyspace RTE:NIP [Not a defect:Unset] "initialized variable due to data flow" */
        ...
        if ((NULL_PTR != firstNode->Next)
        #if(PDUR_CANIF_SUPPORT == STD_ON)
         && (!((LucCanIfPduStatus == CANIF_GW_STOPPED) || (LucCanIfPduStatus == CANIF_GW_ERRVOL)))
        #endif
        )
        {
          ptrNode = firstNode->Next;
          /* Get PduInfoPtr into local pointer */
          LpPduInfoPtr = &(ptrNode->PduInfoPtr);
                        ...
                        NULL_PTR)
          {
            (void)PduR_GaaIfTransmitService[(LpLoTxConfirmation->ucTargetIndex)
                        & PDUR_INDEX_MASK].pTargetLayer
            (ptrNode->TargetPduId, LpPduInfoPtr);
          }
    ...
    #endif /* End of if (PDUR_ROUTINGPATH_GROUP_SUPPORT == STD_ON) */
    {
      ...
      if (PDUR_GATEWAY_MASK == (LucTargetIndex & PDUR_GATEWAY_MASK))
      {
        ...
        if (PDUR_INVALID_BUFFER != LpIfTriggerTransmit->usTxBufferIndex)
        {
          ...
          #if (PDUR_FIFO_TX_BUFFER_SUPPORT == STD_ON)
          firstNode = &PduR_GaaTxFirstNode[LpIfTriggerTransmit->usFirstNodeIndex];
          lastNode = &PduR_GaaTxLastNode[LpIfTriggerTransmit->usFirstNodeIndex];

          /* Check if buffer is empty */
          if ((LpIfTriggerTransmit->ucBufferType == (uint8)PDUR_TT_FIFO) && (NULL_PTR != firstNode->Next))
          {
            ptrNode = firstNode->Next;
                /* Get PduInfoPtr into local pointer */
              ...
              if (NULL_PTR != firstNode->Next)
              {
                                  ...
                                  pTargetLayer != NULL_PTR)
                {
                  ...
                  PduR_GaaIfTransmitService[LucTargetIndex & PDUR_TARGET_INDEX_MASK].
                                 pTargetLayer(ptrNode->TargetPduId,
