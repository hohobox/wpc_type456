...
/* Macro for invalid condition */
#define PDUR_INVALID_PDUID                    ((uint16)0xFFFF)
...
typedef struct PduR_TxNodeType
{
  ...
  /* Next pointer */
  /* polyspace +1 MISRA-C3:20.7 [Not a defect:Low] "use type struct PduR_TxNodeType in define struct PduR_TxNodeType to easy implementation for fifo queue buffer" */
  P2VAR(struct PduR_TxNodeType, AUTOMATIC, PDUR_VAR) Next; 

  /* TargetPduId. Store TxPduId handles by TxConfirmation when resolving FIFO */
  PduIdType TargetPduId;

  /* Status for free buffer */
  PduR_SharedBuffer_StateType BufferStatus;
...
typedef struct
{
  ...
  /* uFirstNodeIdx */
  uint16 uFirstNodeIdx;
         
  /* uFirstNodeNum */
  uint16 uFirstNodeNum;
} PduR_TxSharedBuffer;
#endif
...
#define PDUR_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
extern VAR(PduR_TxNodeType, PDUR_VAR)
                     PduR_GaaTxQueue[PDUR_TX_QUEUE_INDEX_COUNT];
#define PDUR_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
#endif

/* global variable is used as an array of FirstNode for Shared buffer*/
#if (PDUR_SHARED_GW_IF_BUFFER == STD_ON)
#define PDUR_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
extern VAR(PduR_TxNodeType, PDUR_VAR)
                     PduR_GaaTxFirstNode[PDUR_TX_FIRSTNODE_INDEX_COUNT];
#define PDUR_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
#endif

/* global variable is used as an array of LastNode for Shared buffer*/
#if (PDUR_SHARED_GW_IF_BUFFER == STD_ON)
#define PDUR_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
extern VAR(PduR_TxNodeType, PDUR_VAR)
                     PduR_GaaTxLastNode[PDUR_TX_FIRSTNODE_INDEX_COUNT];
#define PDUR_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "MemMap.h"
#endif
