...
#define COM_TXSIG_C_SW_MINOR_VERSION  9
...
#if((COM_TX_ARRAY_ACCESS_FALSE_EXIST == STD_ON) || \
(COM_TX_SIGINGROUP_ARRAY_ACCESS == STD_OFF))
#define COM_START_SEC_CODE
#include "MemMap.h"
...
FUNC(uint8, COM_CODE) Com_SendSignalGroup (Com_SignalGroupIdType SignalGroupId)
{
  /* Pointer to Tx pack structure */
  P2CONST(Com_TxPack, AUTOMATIC, COM_CONST) LpTxPack;
  #if((COM_TX_ARRAY_ACCESS_FALSE_EXIST == STD_ON) || (COM_TX_SIGINGROUP_ARRAY_ACCESS == STD_OFF))
  /* Pointer to Tx signal Group structure */
  P2CONST(Com_TxSigGrp, AUTOMATIC, COM_CONST) LpTxSigGrp;
  #endif
  #if(COM_TX_TP_SUPPORT == STD_ON)
...
FUNC(uint8, COM_CODE) Com_SendSignalGroupArray (Com_SignalGroupIdType SignalGroupId, P2CONST (uint8, AUTOMATIC, COM_CONST) SignalGroupArrayPtr )
{
  ...
  /* Local variable to store return value */
  uint8 LucReturnValue = (uint8)E_OK;
  #if(((COM_TX_SIGGROUP_GREATER_THAN_PENDING == STD_ON) || (COM_TX_SIGGROUP_TRIGONCHANGE == STD_ON) || (COM_TX_SIGGROUP_ARR_GROUPSIG_TRIGONCHANGE == STD_ON)) ||\
      (COM_TX_SIGGROUP_UB == STD_ON) || (COM_TX_SIGGROUP_TMS == STD_ON) || (COM_TX_SIGGROUP_GREATER_THAN_PENDING == STD_ON))
  /* Local variable to store signal group config */
  uint16 LusSigGrpConfig;
  #endif
  #if((COM_TX_SIGGROUP_GREATER_THAN_PENDING == STD_ON) || (COM_TX_SIGGROUP_TRIGONCHANGE == STD_ON) || (COM_TX_SIGGROUP_ARR_GROUPSIG_TRIGONCHANGE == STD_ON))
    ...
    {      
      ...
      #endif
      {
        ...
        if(Com_GaaTxIpduStatus[LpTxSigGrp->ddTxIpduIndex] != (uint16)COM_ZERO)
        {
          ...
          #endif
          {
            #if(COM_TX_SIGGROUP_GREATER_THAN_PENDING == STD_ON)
            if((LusSigGrpProperty) > COM_TX_SIGGRP_PENDING_PROPERTY)
            {
              #if ((COM_TX_IPDU_TMS_SIG_BASED == STD_ON) || \
              (COM_TX_IPDU_REPETITION == STD_ON) || \
           ...
           P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr)
{
  ...
  */
  ...
  #endif
  {
	  #if(((COM_ENABLE_SIGNALGROUP_ARRAY_API == STD_ON) && (COM_TX_SIGINGROUP_ARRAY_ACCESS == STD_ON) && (COM_TX_ARRAY_ACCESS_FALSE_EXIST == STD_ON))|| \
        ((COM_TX_SIGGROUP_TRIGONCHANGE == STD_ON) && (COM_TX_SIGGROUP_GROUPSIG_TRIGONCHANGE == STD_ON)))
  ...
  P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr)
{
  ...
  ((COM_TX_SIG_GREATER_THAN_PENDING == STD_ON) ||(COM_TX_SIGNAL_TRIGONCHANGE == STD_ON)))
  /* Local variable to store signal config */
  uint8 LucSigConfig;
  #endif
  #if((COM_TX_SIG_GREATER_THAN_PENDING == STD_ON) ||(COM_TX_SIGNAL_TRIGONCHANGE == STD_ON))
  /* Local variable to store signal property */
  uint8 LucSigProperty;
  #endif
  #if(COM_TX_SIGNAL_TRIGONCHANGE == STD_ON)
  ...
  */
  ...
  #endif
  {
    ...
    buffer */
    (void)Com_PackRequestWrite(LpTxPack, SignalDataPtr);
    #if((COM_TX_SIGNAL_UB == STD_ON) || (COM_TX_IPDU_TMS_SIG_BASED == STD_ON) ||\
    ((COM_TX_SIG_GREATER_THAN_PENDING == STD_ON) ||(COM_TX_SIGNAL_TRIGONCHANGE == STD_ON)))
    /* Get the signal configuration into local variable */
    LucSigConfig = LpTxSignal->ucTxSigConfig;
    #endif
    ...
    if(Com_GaaTxIpduStatus[LpTxSignal->ddTxIpduIndex] != (uint16)COM_ZERO)
    {
      ...
      #endif
      {
        /* Check whether signal transfer property is triggered or
           transmission mode selection is configured */
        #if(COM_TX_SIG_GREATER_THAN_PENDING == STD_ON)     
        if(LucSigProperty  > COM_TX_SIG_PENDING_PROPERTY)
        {  
          #if ((COM_TX_IPDU_TMS_SIG_BASED == STD_ON) || \
          (COM_TX_IPDU_REPETITION == STD_ON) || \
