...
********************************************************************************
** 2.9.7.0   24-Jan-2024   DH Choi     Internal Redmine #44818                **
** 2.9.5.0   07-Aug-2023   DH Choi     Internal Redmine #42117                **
** 2.8.0     08-Mar-2022   SM Kwon     Internal Redmine #33270                **
...
#define COM_MAIN_C_SW_MINOR_VERSION  9
...
*******************************************************************************/
    ...
    (COM_RX_SIGGROUP_SUBSTITUTE == STD_ON) || \
...
when Rx Timeout is occurred - Fixed by AUTOEVER START*/
...
#endif                         
/* Reset the previous value in MASKED_NEW_DIFFERS_MASKED_OLD filter, 
when Rx Timeout is occurred - Fixed by AUTOEVER END*/
...
FUNC(void, COM_CODE) Com_MainFunctionRx(void)
{
  ...
  #endif
  {
    ...
    #endif
    {
      ...
      #if((COM_RX_SIG_GATEWAY == STD_ON) || (COM_RX_SIGGROUP_GW == STD_ON) || \
       ((COM_RX_IPDU_DEFERRED == STD_ON) \
        &&(((COM_RX_SIGNAL_IMMEDIATENOTIFY == STD_ON)||(COM_RX_SIGGROUP_IMMEDIATENOTIFY)) ||\
            ((COM_RX_SIGNAL == STD_ON) && (COM_RX_SIGGROUP == STD_ON)) ||\
            (COM_RX_SIGGROUP == STD_ON))) || \
            (COM_RX_IPDU_DM == STD_ON))
...
static FUNC(void, COM_CODE) Com_RxUpdateDmTimers(void)
{
  ...
  do
  {
    ...
    {
      LddNoOfRxDm -= LpRxDm->ucRxDmCount;
      LddDmCount += LpRxDm->ucRxDmCount;
      /* Get the pointer to n Rx deadline monitoring structure */
      LddRxDmIndex += LpRxDm->ucRxDmCount;
      LpRxDm = &Com_GaaRxDm[LddRxDmIndex];   
    }
  }while(LddNoOfRxDm != COM_ZERO);
}
#define COM_STOP_SEC_CODE
#include "MemMap.h"
#endif
...
FUNC(void, COM_CODE) Com_MainFunctionTx(void)
{
  ...
  uint8 LucReptSubCount;
    ...
    || (((COM_ENABLE_MDT_CYCLIC == STD_ON) && (COM_TX_IPDU_MINDELAY_ENABLED == STD_ON)) && \
        (COM_TX_IPDU_MDT_COUNT < COM_TX_IPDU_COUNT)))
  uint8 LucTxConfig;
  #endif  
    ...
    do
    {
      ...
      if(Com_GaaTxIpduStatus[LddNoOfTxIpdu] != (uint16)COM_ZERO)
      {
        ...
        #endif
        {
          ...
          #endif
          {
            #if((COM_INVALID_PERIODIC_TIMER_INDEX_EXIST == STD_ON) && (COM_NONE_MESSAGE == STD_ON))
            if(LpTxIpdu->ddTxPeriodicTimerIndex < COM_TX_PERIOD_TIMER_COUNT)
            #endif
            {
              ...
              if(LusTimer == (uint16)COM_ZERO)
              {
                ...
                #endif
                {
                  ...
                  #if(COM_INVALID_REPT_TIMER_INDEX_EXIST == STD_ON)
              ...
              (LucTxMode == COM_TX_MIXEDNTIMES_MODE))
          {
            ...
            if((LusTxIpduStatusBit & COM_TX_REPT_STATUS_MASK) != COM_ZERO)
            {
              ...
              { 
                ...
                #if(COM_EVENT_REPT_COUNT_ZERO_EXIST == STD_ON)
          ...
          #endif
          {
            ...
            #if(COM_TX_IPDU_MDT_COUNT < COM_TX_IPDU_COUNT)
  ...
  #endif
  {
    ...
    if((uint8)E_OK == LddReturnValue)
    {
      Com_GaaTxIpduStatusBit[LddTxIpduId] &=
             (~(COM_TX_RETRY_STATUS_MASK | COM_TX_CONFIRM_STATUS_MASK));
        ...
        if(LusTimer != (uint16)COM_ZERO)
        {
          ...
          if((uint16)COM_ZERO == LusTimer)
          {
            ...
            Com_GaaTxIpduStatusBit[LddTxIpduIndex] &=
                 (~(COM_TX_RETRY_STATUS_MASK | COM_TX_REQ_STATUS_MASK));
            SchM_Exit_Com_TX_IPDU_STATUS_PROTECTION_AREA();
            #if((COM_TX_IPDU_TMS_SIG_BASED == STD_ON)||(COM_TX_IPDU_TMS_EXPLICIT == STD_ON))
            ...
            {
              if(Com_GaaTxIpdu[LddTxIpduIndex].ddTxReptIndex < COM_TX_REPT_COUNT)
              { 
...
#endif
{
  ...
  PduLengthType LddSignalMemSize;
       ...
       (COM_RX_SIGNAL_FILTER == STD_ON) || (COM_RX_SIGNAL_INVALID == STD_ON)\
                                      ||(COM_RX_SIGNAL_UBDMREPLACE == STD_ON)\
                                            ||((COM_RX_SIGNAL_NOTIFY == STD_ON) && (COM_RX_SIGNAL_NOTIFY_NOT_CONFIGURED == STD_ON)))
  ...
  do
  {
    if(LddLength >= LpRxSignal->ddSignalEndByte)
    {
      ...
      #endif
      {
        ...
        #if (COM_RX_SIGNAL_NOTIFY_NOT_CONFIGURED == STD_ON)
...
**                                                 SchM_Exit_xxx()            **
*******************************************************************************/
#if((COM_TX_IPDU == STD_ON) && (COM_TX_IPDU_TMS_EXPLICIT == STD_ON))
#define COM_START_SEC_CODE
#include "MemMap.h"
FUNC(void, COM_CODE) Com_SwitchIpduTxMode(PduIdType PduId,
  boolean Mode)
{
  #if(((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON) || (COM_NONE_MESSAGE == STD_ON)) || \
      (((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON)) && \
      (((COM_PERIODIC_MESSAGE == STD_ON) || (COM_MIXED_MESSAGE == STD_ON)) || ((COM_DIRECT_MESSAGE == STD_ON) && (COM_DIRECT_NTIMES_MESSAGE == STD_ON)))))
  /* Pointer to Tx mode structure */
  P2CONST(Com_TxMode, AUTOMATIC, COM_CONST) LpTxMode;
  boolean LblValidTxModeIdx = COM_TRUE;
  #endif
  #if ((((COM_TX_IPDU_MINDELAY_ENABLED == STD_ON) && (COM_TX_IPDU_MDT_COUNT < COM_TX_IPDU_COUNT))) ||\
      (COM_TX_IPDU_REPETITION == STD_ON) || \
      (((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON) || (COM_NONE_MESSAGE == STD_ON)) || \
        (((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON)) && \
        (((COM_PERIODIC_MESSAGE == STD_ON) || (COM_MIXED_MESSAGE == STD_ON)) || ((COM_DIRECT_MESSAGE == STD_ON) && (COM_DIRECT_NTIMES_MESSAGE == STD_ON))))))  
  ...
  #endif
  {
    ...
    if(Com_GaaTxIpduStatus[PduId] > (uint16)COM_ZERO)
    {
      ...
      if((Com_GaaTxIpduStatusBit[PduId] & COM_TX_TMS_STATUS_MASK) != Mode)
      {
        ...
        (((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON)) && \
        (((COM_PERIODIC_MESSAGE == STD_ON) || (COM_MIXED_MESSAGE == STD_ON)) || ((COM_DIRECT_MESSAGE == STD_ON) && (COM_DIRECT_NTIMES_MESSAGE == STD_ON)))))
        if(LblValidTxModeIdx == COM_TRUE)
        #endif
        {
          /* Resetting timers if the mode is PERIODIC or MIXED (COM244)*/
          #if(((COM_PERIODIC_MESSAGE == STD_ON) || (COM_MIXED_MESSAGE == STD_ON) || (COM_MIXED_NTIMES_MESSAGE == STD_ON))\
              && ((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON) || (COM_NONE_MESSAGE == STD_ON)))
          if((LpTxMode->ucTxMode <= COM_TX_MIXEDNTIMES_MODE)  &&
              (LpTxMode->ucTxMode != COM_TX_NONE_MODE))
          #endif
          ...
          #if((COM_DIRECT_MESSAGE == STD_ON) || (COM_DIRECT_NTIMES_MESSAGE == STD_ON))
          #if((COM_PERIODIC_MESSAGE == STD_ON) || (COM_MIXED_MESSAGE == STD_ON) || (COM_MIXED_NTIMES_MESSAGE == STD_ON) || (COM_NONE_MESSAGE == STD_ON))
                          ...
                          P2CONST(Com_RxDm, AUTOMATIC, COM_CONST) LpRxDm)
{
  ...
  if(LucNoOfSigCount != COM_ZERO)
  {
    ...
    LucSigReplaceIndex = LpRxDm->usSigReplaceIndex;
    LpSigReplace = &Com_GaaSigReplace[LucSigReplaceIndex];
    do
    {
      LpPackTimeout = &Com_GaaTxPack[LpSigReplace->ddTxPackIndex];

      if(LpSigReplace->pInitValue != NULL_PTR)
      {
        /* Invoke write function to pack the initial value into
         I-PDU/signal buffer */
        (void)Com_PackRequestWrite(LpPackTimeout, LpSigReplace->pInitValue);
      }
      else if(LpSigReplace->pSubstituteValue != NULL_PTR)
      {
        /* Invoke write function to pack the substitute value into
         I-PDU/signal buffer */
        (void)Com_PackRequestWrite(LpPackTimeout, LpSigReplace->pSubstituteValue);
      }
      else
      {
        /* Do Nothing */
  ...
  {
    /* Get the pointer to LpSigReplace structure */
    LucSigReplaceIndex = LpRxDm->usSigGrpReplaceIndex;
    ...
    do
    {
      ...
      if(LpSigGrpReplace->pInitValue != NULL_PTR)
      {
        /* Invoke write function to pack the initial value into
          I-PDU/signal buffer */
        (void)Com_PackSigGrp(LpPackTimeout, LpSigGrpReplace->pInitValue, LpPackTimeout->pWrBuffer, LpRxSigGroup->pMaskingBuffer);
      }
      else if(LpSigGrpReplace->pSubstituteValue != NULL_PTR)
      {
        /* Invoke write function to pack the substitute value into
          I-PDU/signal buffer */
        (void)Com_PackSigGrp(LpPackTimeout, LpSigGrpReplace->pSubstituteValue, LpPackTimeout->pWrBuffer, LpRxSigGroup->pMaskingBuffer);
      }      
      else
      {
        /* Do Nothing */
