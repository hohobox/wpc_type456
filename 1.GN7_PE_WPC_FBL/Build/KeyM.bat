...
SET BSWMDT=%PRJ_DIR%\Static_Code\Modules\KeyM_R44\generator\Bswmdt_KeyM.template
SET SWCDT=%PRJ_DIR%\Static_Code\Modules\KeyM_R44\generator\Swcdt_Bsw_KeyM.template

:: Run module

IF %XML_GEN%==0 %FW_PATH%\mobilgene_c_generator.exe -B %BASE_DIR% -M %MODULES% -T %MODULE_TOP_PATH% -O %OUT_FOLDER% -I %INPUT_FILE_YML% -L -G -V -configuration %FW_PATH%\%MODULES%_%RANDOM_NUMBER%
...
%FW_PATH%\mobilgene_c_generator.exe -B %BASE_DIR% -M %MODULES% -T %MODULE_TOP_PATH% -O %OUT_FOLDER% -I %INPUT_FILE_YML% -XML -SWCD %SWCDT% -L -G -configuration %FW_PATH%\%MODULES%_%RANDOM_NUMBER%
